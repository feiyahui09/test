...
public class ChatHelper {
	...
	public void init(Context context, final String serverIP, final int serverPort, int uid, String rid,
			final String authToken, String name,
...
//					client = new PomeloClient(msg.getString("host"), msg.getInt("port"));
							...
							client.init(ioCallback, clientCallback);
							// 真正分配到个服务器，
							client.request("connector.entryHandler.enter", new JSONObject().put("auth_token",
											authToken),
									new DataCallBack() {
										public void responseData(JSONObject msg) {
											// handle data here
							...
							client.on("onZmax", new DataListener() {
								@Override
								public void receiveData(DataEvent event) {
									JSONObject msg = event.getMessage();
									Log.i("onKick: " + msg.toString());
	...
	public void send(String content, boolean isImg) {
		try {
			...
			client.inform("chat.chatHandler.send",
					new JSONObject().put("content", content).put("name", name).put("gender", gender).put("type",
							isImg ? "image" : "text"));
		} catch (Exception e) {
			e.printStackTrace();
